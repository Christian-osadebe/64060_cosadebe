...
```
On calibrated scale of 0 (perfect) to 1 (worst) for Brier Score, Brier_Score_B:
0.0843 is closer to 0 than Brier_Score_E: 0.0844 - that is, the probability 
obtained from part B: 0.1071429 is more accurate than probability obtained from 
part E: 0.1105637.
...
Predicted_Train_label_ <- predict(nb_model, Train, type = "raw")

# Create dataframe for returned probablitites
df <- data.frame(Predicted_Train_label_)
...
head(df_sub,1)
```
The probability value from the model output for P(Loan = 1 | CC = 1, Online = 1): 0.1156935, 
is slightly greater than the value obtained using the Naive Bayes probability formula in part E: 0.1105637.
